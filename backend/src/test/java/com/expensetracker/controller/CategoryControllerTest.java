...
class CategoryControllerTest {
    ...
    void renameCategory_duplicateName_returns409() throws Exception {
        ...
        mockMvc.perform(put("/api/categories/" + categoryBId)
                .header(USER_ID_HEADER, "1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(categoryJson("IntTest Dup A")))
            .andExpect(status().isConflict())
    ...
    void deleteCategory_withTransactions_returns409() throws Exception {
        ...
        jdbcTemplate.update(
            ...
            1L, categoryId, "Test Transaction", new BigDecimal("25.50"), "USD", LocalDate.of(2026, 3, 15)
        );
        ...
        mockMvc.perform(delete("/api/categories/" + categoryId)
                .header(USER_ID_HEADER, "1"))
            .andExpect(status().isConflict())
            .andExpect(jsonPath("$.error", is("Cannot delete category with existing transactions")));
    }
