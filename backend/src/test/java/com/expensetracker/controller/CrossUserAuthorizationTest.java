 ...
 *
 * Verifies that:
 * - User A's categories are invisible to User B
 * - User B cannot access/modify/delete User A's categories by ID
 * - User A's transactions are invisible to User B
 * - User B cannot access/modify/delete User A's transactions by ID
 * - User B cannot create a transaction using User A's category ID
 * - The FakeAuthFilter correctly switches users via X-User-Id header
 *
 * Uses seeded fake users: User 1 (id=1) and User 2 (id=2) from migrations V100 and V101.
 */
@SpringBootTest
...
class CrossUserAuthorizationTest {
    ...
    class FakeAuthFilterTests {
        ...
        void defaultUserIsUser1() throws Exception {
            ...
            mockMvc.perform(get("/api/categories")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.name == 'AuthTest Default User Cat')]").doesNotExist());
        }
    ...
    class CategoryIsolationTests {
        ...
        void userB_cannotRenameUserACategory() throws Exception {
            ...
            mockMvc.perform(put("/api/categories/" + userACatId)
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        void userB_cannotDeleteUserACategory() throws Exception {
            ...
            mockMvc.perform(delete("/api/categories/" + userACatId)
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isNotFound());
    ...
    class TransactionIsolationTests {
        ...
        void setUpCategories() throws Exception {
            ...
            MvcResult catA = mockMvc.perform(post("/api/categories")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            MvcResult catB = mockMvc.perform(post("/api/categories")
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        @Test
        @DisplayName("GET /api/transactions as User B returns empty when only User A has transactions")
        void userB_listTransactions_doesNotSeeUserAData() throws Exception {
            // User A creates transactions
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Groceries", "52.75", "2026-03-15", userACategoryId, "Weekly shopping")))
                .andExpect(status().isCreated());

            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Gas", "40.00", "2026-03-14", userACategoryId, null)))
                .andExpect(status().isCreated());
        ...
        void userB_cannotUpdateUserATransaction() throws Exception {
            ...
            MvcResult result = mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Update Target", "50.00", "2026-03-15", userACategoryId, null)))
                .andExpect(status().isCreated())
                .andReturn();
            ...
            mockMvc.perform(put("/api/transactions/" + userATxnId)
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Hacked Title", "999.00", "2026-03-20", userBCategoryId, "hacked")))
                .andExpect(status().isNotFound());
        ...
        void userB_cannotDeleteUserATransaction() throws Exception {
            ...
            MvcResult result = mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Delete Target", "75.00", "2026-03-15", userACategoryId, null)))
                .andExpect(status().isCreated())
                .andReturn();
            ...
            mockMvc.perform(delete("/api/transactions/" + userATxnId)
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isNotFound());
        ...
        void userB_cannotCreateTransactionWithUserACategoryId() throws Exception {
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Cross Category", "25.00", "2026-03-15", userACategoryId, null)))
                .andExpect(status().isNotFound())
            ...
            mockMvc.perform(get("/api/transactions")
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.title == 'AuthTest Cross Category')]").doesNotExist());
        }
        ...
        void userB_cannotUpdateTransactionWithUserACategoryId() throws Exception {
            ...
            MvcResult result = mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest UserB Own Txn", "30.00", "2026-03-15", userBCategoryId, null)))
                .andExpect(status().isCreated())
                .andReturn();
            ...
            mockMvc.perform(put("/api/transactions/" + userBTxnId)
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest UserB Own Txn", "30.00", "2026-03-15", userACategoryId, null)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").isNotEmpty());
        }
    }
    ...
    class EndToEndIsolationTests {
        ...
        void fullIsolationScenario() throws Exception {
            ...
            MvcResult txnResult1 = mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest E2E Groceries", "52.75", "2026-03-15", userACatId1, "Weekly")))
                .andExpect(status().isCreated())
                .andReturn();
            Long userATxnId1 = extractId(txnResult1);

            MvcResult txnResult2 = mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest E2E Bus Fare", "3.50", "2026-03-14", userACatId2, null)))
                .andExpect(status().isCreated())
                .andReturn();
            ...
            mockMvc.perform(put("/api/categories/" + userACatId1)
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            mockMvc.perform(delete("/api/categories/" + userACatId1)
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isNotFound());
            ...
            MvcResult userBCatResult = mockMvc.perform(post("/api/categories")
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            mockMvc.perform(put("/api/transactions/" + userATxnId1)
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest E2E Hacked Txn", "999.00", "2026-03-20", userBCatId, "hacked")))
                .andExpect(status().isNotFound());

            // === Step 8: User B cannot delete User A's transaction by ID ===
            mockMvc.perform(delete("/api/transactions/" + userATxnId1)
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isNotFound());

            mockMvc.perform(delete("/api/transactions/" + userATxnId2)
                    .header(USER_ID_HEADER, USER_B))
                .andExpect(status().isNotFound());
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_B)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest E2E Cross Cat", "10.00", "2026-03-15", userACatId1, null)))
                .andExpect(status().isNotFound());
    ...
    class TransactionFilterIsolationTests {
        ...
        void userB_cannotSearchUserATransactions() throws Exception {
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Secret Purchase", "100.00", "2026-03-15", userACatId, "confidential notes")))
                .andExpect(status().isCreated());
            ...
            mockMvc.perform(get("/api/transactions")
                    ...
                    .param("search", "Secret"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.title == 'AuthTest Secret Purchase')]").doesNotExist());
        ...
        void userB_cannotFilterByUserACategoryId() throws Exception {
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Filtered Item", "50.00", "2026-03-15", userACatId, null)))
                .andExpect(status().isCreated());
            ...
            mockMvc.perform(get("/api/transactions")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.title == 'AuthTest Filtered Item')]").doesNotExist());
        }
        ...
        void userB_cannotFilterByDateRange() throws Exception {
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest March Item", "50.00", "2026-03-15", userACatId, null)))
                .andExpect(status().isCreated());
        ...
        void userB_cannotFilterByAmountRange() throws Exception {
            ...
            mockMvc.perform(post("/api/transactions")
                    .header(USER_ID_HEADER, USER_A)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(transactionJson("AuthTest Expensive Item", "500.00", "2026-03-15", userACatId, null)))
                .andExpect(status().isCreated());
            ...
            mockMvc.perform(get("/api/transactions")
                    ...
                    .param("amountMax", "600"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.title == 'AuthTest Expensive Item')]").doesNotExist());
        }
    }
}
