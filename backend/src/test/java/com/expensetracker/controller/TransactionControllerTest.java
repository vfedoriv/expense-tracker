...
class TransactionControllerTest {
    ...
    void setUp() throws Exception {
        ...
        MvcResult cat1Result =
                mockMvc.perform(
                        ...
                        .andReturn();
        user1CategoryId =
                ((Number) JsonPath.read(cat1Result.getResponse().getContentAsString(), "$.id"))
                        .longValue();

        MvcResult cat2Result =
                mockMvc.perform(
                                post("/api/categories")
                        ...
                        .andReturn();
        user2CategoryId =
                ((Number) JsonPath.read(cat2Result.getResponse().getContentAsString(), "$.id"))
                        .longValue();
    }
    ...
    void createTransaction_missingDate_returns400() throws Exception {
        String json =
                "{\"title\": \"TxnTest No Date\", \"amount\": 50.00, \"categoryId\": "
                        + user1CategoryId
                        + "}";
        mockMvc.perform(
                        post("/api/transactions")
    ...
    void createTransaction_missingCategoryId_returns400() throws Exception {
        String json =
                "{\"title\": \"TxnTest No Cat\", \"amount\": 50.00, \"transactionDate\": \"2026-03-15\"}";
        mockMvc.perform(
                        post("/api/transactions")
                                .header(USER_ID_HEADER, "1")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void listTransactions_returnsOnlyAuthenticatedUsersTransactions() throws Exception {
        ...
        mockMvc.perform(get("/api/transactions").header(USER_ID_HEADER, "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.title == 'TxnTest User2 Item')]").doesNotExist());

        // User 2 should not see user 1's transactions
        mockMvc.perform(get("/api/transactions").header(USER_ID_HEADER, "2"))
                .andExpect(status().isOk())
    ...
    void listTransactions_combinedFilters() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$[?(@.title == 'TxnTest Combined WrongDate')]").doesNotExist());
    }
    ...
    void deleteTransaction_returns204() throws Exception {
        ...
        // Delete
        mockMvc.perform(delete("/api/transactions/" + txnId).header(USER_ID_HEADER, "1"))
    ...
    void deleteTransaction_crossUser_returns404() throws Exception {
        ...
        mockMvc.perform(delete("/api/transactions/" + txnId).header(USER_ID_HEADER, "2"))
                .andExpect(status().isNotFound());
    }
}
