...
/**
 * WireMock-based OAuth2 integration tests that mock the full provider callback flow
 * (token exchange + userinfo endpoints). Uses a real embedded server and real HTTP
 * client to simulate the full OAuth2 authorization code flow.
 */
...
class OAuth2WireMockFlowTest {
    ...
    @Test
    @DisplayName("GitHub full OAuth2 callback flow: token exchange, userinfo, user provisioned in DB")
    void githubFullOAuth2CallbackFlow() throws Exception {
        // Stub GitHub token endpoint
        wireMock.stubFor(post(urlPathEqualTo("/login/oauth/access_token"))
            .willReturn(okJson("""
                {
        ...
        wireMock.stubFor(get(urlPathEqualTo("/api/user"))
            .willReturn(okJson("""
                {
                    "id": 67890,
        ...
        assertTrue(userRepository.findByProviderAndProviderUserId("github", "67890").isEmpty(),
        ...
        assertUserMeEndpoint(client, sessionCookie, "github", "WireMock GitHub User", "wm@github.com");
    ...
    @Test
    @DisplayName("Google full OIDC callback flow: token exchange with id_token, userinfo, user provisioned in DB")
    void googleFullOidcCallbackFlow() throws Exception {
        // No user should exist initially
        assertTrue(userRepository.findByProviderAndProviderUserId("google", "google-12345").isEmpty(),
            "No Google user should exist before the flow");

        // Execute Google OIDC flow using helper
        HttpClient client = createHttpClient();
        String sessionCookie = executeGoogleOidcFlow(client,
            "google-12345", "wm@google.com", "WireMock Google User", "https://example.com/g.jpg");

        // Verify user was provisioned in the DB
        Optional<User> dbUser = userRepository.findByProviderAndProviderUserId("google", "google-12345");
        ...
        // Verify /api/users/me returns the provisioned user using the session
        assertUserMeEndpoint(client, sessionCookie, "google", "WireMock Google User", "wm@google.com");
    ...
    void firstLoginCreatesUser_secondLoginReusesSameRecord() throws Exception {
        ...
        stubGitHubEndpoints(67890, "wiremock-gh-user", "WireMock GitHub User",
            "wm@github.com", "https://example.com/gh.jpg");

        // First login
        HttpClient client1 = createHttpClient();
        String session1 = executeOAuth2Flow(client1, "github");

        Optional<User> userAfterFirst = userRepository.findByProviderAndProviderUserId("github", "67890");
        ...
        Optional<User> userAfterSecond = userRepository.findByProviderAndProviderUserId("github", "67890");
        ...
        long count = userRepository.findAll().stream()
            .filter(u -> "github".equals(u.getProvider()) && "67890".equals(u.getProviderUserId()))
            .count();
        assertEquals(1, count, "DB should have exactly one user record with provider=github, providerUserId=67890");
    ...
    void differentProviders_sameEmail_createSeparateAccounts() throws Exception {
        ...
        stubGitHubEndpoints(99901, "shared-gh-login", "Shared GitHub User",
            "shared@example.com", "https://example.com/gh-shared.jpg");

        HttpClient ghClient = createHttpClient();
        String ghSession = executeOAuth2Flow(ghClient, "github");

        Optional<User> githubUser = userRepository.findByProviderAndProviderUserId("github", "99901");
        ...
        String googleSession = executeGoogleOidcFlow(googleClient,
            "shared-google-user", "shared@example.com", "Shared Google User",
            "https://example.com/g-shared.jpg");

        Optional<User> googleUser = userRepository.findByProviderAndProviderUserId("google", "shared-google-user");
        ...
        assertNotEquals(githubUser.get().getId(), googleUser.get().getId(),
    ...
    /**
     * Executes the full Google OIDC flow with proper nonce handling.
     * Must be called with a fresh (no stubs) or reset WireMock.
     * Sets up stubs after capturing the nonce from the authorization redirect.
     */
    private String executeGoogleOidcFlow(HttpClient client, String subject, String email,
                                          String name, String picture) throws Exception {
        // Step 1: Stub only the JWKS endpoint (needed during token validation)
        wireMock.stubFor(get(urlPathEqualTo("/oauth2/v3/certs"))
            .willReturn(okJson(createJwksJson())));

        // Stub userinfo endpoint
        wireMock.stubFor(get(urlPathEqualTo("/oauth2/v3/userinfo"))
        ...
        assertEquals(302, authResponse.statusCode(),
            "Google OAuth2 authorization should redirect (302)");

        String location = authResponse.headers().firstValue("Location").orElseThrow(
            () -> new AssertionError("No Location header on Google authorization redirect"));
        ...
        wireMock.stubFor(post(urlPathEqualTo("/oauth2/v4/token"))
            .willReturn(okJson(String.format("""
                {
        ...
        HttpRequest callbackRequest = HttpRequest.newBuilder()
            .uri(URI.create(baseUrl + "/login/oauth2/code/google"
        ...
        assertTrue(callbackResponse.statusCode() == 302 || callbackResponse.statusCode() == 200,
            "Google callback should succeed, got: " + callbackResponse.statusCode()
    ...
    /**
     * Executes the full OAuth2 authorization code flow for the given provider.
     * Returns the session cookie value for subsequent authenticated requests.
     */
    ...
    /**
     * Executes the full OAuth2 authorization code flow with optional nonce consumer.
     * For OIDC providers (Google), captures the nonce from the auth redirect to include
     * in the WireMock token stub.
     */
    private String executeOAuth2FlowInternal(HttpClient client, String provider,
                                              java.util.function.Consumer<String> nonceConsumer) throws Exception {
        ...
        String location = authResponse.headers().firstValue("Location").orElseThrow(
            () -> new AssertionError("No Location header on authorization redirect"));
        ...
        HttpRequest callbackRequest = HttpRequest.newBuilder()
            .uri(URI.create(baseUrl + "/login/oauth2/code/" + provider
                + "?code=mock-auth-code&state=" + state))
            .GET()
            .build();
        ...
        assertTrue(callbackResponse.statusCode() == 302 || callbackResponse.statusCode() == 200,
            "Callback should succeed with redirect or OK, got: " + callbackResponse.statusCode());
    ...
    private String extractSessionCookie(HttpClient client) throws Exception {
        ...
        return cm.getCookieStore().getCookies().stream()
            ...
            .findFirst()
            .map(c -> c.getValue())
            .orElseThrow(() -> new AssertionError("No JSESSIONID cookie found after OAuth2 flow"));
    }

    /**
     * Calls GET /api/users/me with the session cookie and verifies the response.
     */
    private void assertUserMeEndpoint(HttpClient client, String sessionCookie,
                                       String expectedProvider, String expectedDisplayName,
                                       String expectedEmail) throws Exception {
        String meResponse = getUserMe(client, sessionCookie);
        assertTrue(meResponse.contains("\"provider\":\"" + expectedProvider + "\""),
            "Response should contain provider=" + expectedProvider + ", got: " + meResponse);
        assertTrue(meResponse.contains("\"displayName\":\"" + expectedDisplayName + "\""),
            "Response should contain displayName=" + expectedDisplayName + ", got: " + meResponse);
        assertTrue(meResponse.contains("\"email\":\"" + expectedEmail + "\""),
    ...
    private String getUserMe(HttpClient client, String sessionCookie) throws Exception {
        ...
        assertEquals(200, meResponse.statusCode(),
            "GET /api/users/me should return 200 with valid session, got: " + meResponse.statusCode()
     ...
     */
    private void stubGitHubEndpoints(int userId, String login, String name,
    ...
    /**
     * Creates a signed JWT id_token for Google OIDC.
     * Includes the nonce if provided (required for OIDC nonce validation).
     */
    private String createSignedIdToken(String subject, String email, String name, String picture,
