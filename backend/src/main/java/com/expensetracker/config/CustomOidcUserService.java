...
/**
 * Custom OIDC UserService that handles Google login (OpenID Connect).
 * Extracts provider + provider_user_id (sub claim), checks if user exists in DB,
    ...
    public OidcUser loadUser(OidcUserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        User user = userRepository.findByProviderAndProviderUserId(registrationId, providerUserId)
            .orElseGet(() -> {
                ...
                newUser.setAvatarUrl(avatarUrl != null ? avatarUrl.toString() : null);
